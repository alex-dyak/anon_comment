...
/**
 * @file
 * Contains \Drupal\anon_comment\Controller\AnonCommentController.
 */

namespace Drupal\anon_comment\Controller;
 ...
 */
class AnonCommentController extends ControllerBase {
  ...
  public function adminPage(Request $request, $type = 'new') {
    if ($request->request->get('operation') == 'delete' && $request->request->get('comments')) {
      return $this->formBuilder->getForm('\Drupal\anon_comment\Form\AnonConfirmDeleteMultiple', $request);
    }
    else {
      return $this->formBuilder->getForm('\Drupal\anon_comment\Form\AnonCommentAdminOverview', $type);
    }
  }
